...
const applyTheme = (themeInput = DEFAULT_THEME) => {
  ...
  document.body.style.background = `radial-gradient(circle at top left, ${rgba(theme.accent, 0.12)}, transparent 32%), radial-gradient(circle at top right, ${rgba(theme.accent, 0.09)}, transparent 26%), linear-gradient(180deg, ${theme.bg} 0%, ${shiftHex(theme.bg, -6)} 100%)`;
};
...
const injectBrand = () => {
  $$('[data-brand-lockup]').forEach((slot) => {
    ...
    slot.innerHTML = `
      <a href="index.html" class="brand-lockup text-only ${compact ? 'compact' : ''}">
        <span class="brand-copy">
          <strong>ReimuruABT</strong>
          <span>${compact ? 'Premium student AI' : 'Study system'}</span>
        </span>
      </a>
    `;
  });
...
const renderDashboard = () => {
  ...
  if (!getActiveUser()) {
    mountGuestGate(
      ...
      'Create an account to unlock saved history, watermark-free exports, and your paid GPT plans.'
    );
    return;
  }
  ...
  target.innerHTML = `
    <section class="hero-shell">
      ...
      <h2>${escapeHtml(user.name)}, your study system is behaving.</h2>
    ...
    <div class="metric-grid">
      <article class="metric-card card">
        ...
        <p class="metric-caption">Every result that actually mattered enough to keep.</p>
      </article>
      <article class="metric-card card">
...
const initTimerInteractions = () => {
  ...
  $('[data-exam-pause]')?.addEventListener('click', () => pauseExamTimer(true));
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && examTimerState.running && !examTimerState.paused) {
...
const initSettingsPage = () => {
  ...
  if (themeForm) {
    ...
    themeForm.addEventListener('submit', (event) => {
      ...
      saveScopedSettings({ ...current, theme: nextTheme });
    ...
    $('[data-theme-reset]')?.addEventListener('click', () => {
      const current = getScopedSettings();
      saveScopedSettings({ ...current, theme: DEFAULT_THEME });
...
const initAuthForms = () => {
  ...
  $$('[data-google-soon]').forEach((button) => {
    button.addEventListener('click', (event) => {
      event.preventDefault();
      setFormStatus(status, 'info', 'Google sign-in can be wired next. This button is ready for it.');
    });
  });
